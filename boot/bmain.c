...
{
    void (*voidfunc) (void) = addr;
    irqmask_t irq = (NVIC_IRQ_MAX - 1);
    dprintf("%s() :\n", __func__);
    irq_save(&irq);
    dev_deinit();
    boot_reloc_vectortable((uint32_t)addr);
    voidfunc();
    assert(0);
}

uint32_t g_app_program_addr = ADDR_FLASH_SECTOR_6;
...
static int boot_handle_bins (pane_t *pane, component_t *com, void *user)
{
    ...
    dprintf("Cache bin : \'%s\'", boot_bin_selected->name);
    bindata = cache_bin(boot_bin_selected, &binsize);
    dprintf("Cache done : %p of size %u", bindata, binsize);
    if (!bindata) {
        return 0;
    }
